...
def _qini(responses_control, responses_target, n_control, n_target):
    if n_control == 0:
...
def _uplift(responses_control, responses_target, n_control, n_target):
    if (n_control != 0 and n_target != 0):
        return (n_control + n_target) * ( (responses_target / n_target) - (responses_control / n_control) )
    else:
        return 0
...
def qini_curve(y_true, d_pred, group, n_nodes=None):
    if n_nodes is None:
        n_nodes = min(len(y_true) + 1, 201)
    ...
    xs = [int(i) for i in np.linspace(0, len(y_true), n_nodes)]
    ys = [_qini(*cumulative_responses[x]) for x in xs]
