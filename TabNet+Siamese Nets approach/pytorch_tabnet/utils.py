...
class TorchDataset(Dataset):
    ...
    def __getitem__(self, index):
        x, t, y = self.x[index], self.t[index], self.y[index]
        return x, t, y
...
def create_dataloaders(
    X_train, T_train, y_train, eval_set, weights, batch_size, num_workers, drop_last, pin_memory
):
    """
    ...
    train_dataloader = DataLoader(
        TorchDataset(X_train.astype(np.float32), T_train.astype(np.float32), y_train),
